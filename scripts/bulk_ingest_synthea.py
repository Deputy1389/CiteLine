"""
Bulk Ingest and Stress Test for Synthea Corpus.
Processes 132 Synthea patient PDFs and measures extraction performance.
"""
import os
import sys
import uuid
...
def main():
    ...
    print(f"Found {len(pdf_files)} PDFs for Synthea bulk stress test.")
    ...
    with get_session() as session:
        ...
        matter_id = "synthea_bulk_stress_test"
        matter = session.query(Matter).filter_by(id=matter_id).first()
        if not matter:
            matter = Matter(id=matter_id, title="Synthea Bulk Stress Test", firm_id="demo_firm")
            session.add(matter)
            session.commit()
        ...
        with get_session() as session:
            ...
            if not doc:
                doc = SourceDocument(
                    ...
                    sha256="b" * 64, # New unique sha for synthea
    ...
    print("\n" + "="*40)
    print("SYNTHEA BULK STRESS TEST RESULTS")
    print("="*40)
