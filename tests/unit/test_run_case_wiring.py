...
def test_run_case_uses_checklist_as_single_pass_source(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr(rc, "build_attorney_readiness_report", lambda report_text, ctx: {"attorney_ready_pass": True, "attorney_ready_score_0_100": 95, "failures": [], "metrics": {}})
...
def test_run_case_requires_luqa_and_qa_for_overall_pass(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr(rc, "build_attorney_readiness_report", lambda report_text, ctx: {"attorney_ready_pass": True, "attorney_ready_score_0_100": 98, "failures": [], "metrics": {}})
    monkeypatch.setattr(rc, "write_litigation_checklist", lambda path, checklist: path.write_text(json.dumps(checklist), encoding="utf-8"))
    monkeypatch.setattr(rc, "_write_fail_cover_pdf", lambda out_pdf, checklist, luqa=None, attorney=None: None)
...
def test_run_case_requires_attorney_readiness_for_overall_pass(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr(rc, "build_luqa_report", lambda report_text, ctx: {"luqa_pass": True, "luqa_score_0_100": 100, "failures": [], "metrics": {}})
    monkeypatch.setattr(rc, "build_attorney_readiness_report", lambda report_text, ctx: {"attorney_ready_pass": False, "attorney_ready_score_0_100": 60, "failures": [{"code": "AR_FAIL"}], "metrics": {}})
    monkeypatch.setattr(rc, "write_litigation_checklist", lambda path, checklist: path.write_text(json.dumps(checklist), encoding="utf-8"))
    monkeypatch.setattr(rc, "_write_fail_cover_pdf", lambda out_pdf, checklist, luqa=None, attorney=None: None)

    payload = rc.run_case(input_pdf=input_pdf, case_id="case3", run_label="run3")
    assert payload["overall_pass"] is False
    assert payload["qa_pass"] is True
